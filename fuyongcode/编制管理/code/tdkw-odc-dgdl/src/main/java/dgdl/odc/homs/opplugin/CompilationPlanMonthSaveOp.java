 ...
 * @author lzf
 * @date 2023/9/7 15:33
 * @description 月度编制计划 保存和启用时校验: 同一编制规划单位，编制计划之间开始月份与结束月份不得有交集
 */
public class CompilationPlanMonthSaveOp extends AbstractOperationServicePlugIn {
    private static Log logger = LogFactory.getLog(CompilationPlanMonthSaveOp.class);
    ...
    public void onAddValidators(AddValidatorsEventArgs e) {
        // 添加自定义的校验器
        e.addValidator(new CompilationPlanMonthSaveOpVal());
    }
}
 ...
 * 自定义校验器
 * */
class CompilationPlanMonthSaveOpVal extends AbstractValidator {
    private static final Log logger = LogFactory.getLog(CompilationPlanMonthSaveOpVal.class.getName());
    ...
    public void validate() {
        ...
        for (ExtendedDataEntity dataEntity : dataEntities) {
            ...
            logger.info("CompilationPlanMonthSaveOpVal.objects.length:"+objects.length);
            ...
            if(numbers.size()>0){
                logger.info("CompilationPlanMonthSaveOpVal.numbers:"+ SerializationUtils.toJsonString(numbers));
            }
            ...
            for (DynamicObject object : objects) {
                ...
                if (startMonthStr >= oldStartMonthStr && startMonthStr <= oldEndMonthStr){
                    logger.info("CompilationPlanMonthSaveOpVal.单据编号:"+object.getString("number"));
