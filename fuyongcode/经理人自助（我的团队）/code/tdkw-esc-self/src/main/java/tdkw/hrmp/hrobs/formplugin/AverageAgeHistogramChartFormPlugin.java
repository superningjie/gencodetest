 ...
 * @Description 人员分析 平均年龄 柱状图表单插件
 * @Demander xxx
 * @Document PC端人力自助需规V0.3_0619(2)、https://www.kdocs.cn/l/cdDKeoQxzuPH
 * @Basedata tdkw_personanalysis
 * @Version 1.0
 **/
public class AverageAgeHistogramChartFormPlugin extends AbstractFormPlugin implements ClickListener {
    private Log logger = LogFactory.getLog(AverageAgeHistogramChartFormPlugin.class);
    ...
    public void click(EventObject evt) {
        ...
        if (key.equals("tdkw_ageavgchartap")) {
            ...
            HistogramChart histogramChart = this.getControl("tdkw_ageavgchartap");
...
//            DynamicObject orgObject = QueryServiceHelper.queryOne("haos_adminorghr", "id,name", orgFilter.toArray());
            ...
            yDatas.add(ageSum / dirOrgPersons.size());
...
//            this.getPageCache().put(this.getClass().getName() + dirOrg.getString("name") + "(直属)", dirOrgPersons.toString());
        ...
        if (dirOrgPersonAll.size() != 0) {
            ...
            yDatas.add(ageSums / dirOrgPersonAll.size());
        }
...
//        System.out.println("平均年龄耗时" + (System.currentTimeMillis() - start));
