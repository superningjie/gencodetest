...
import tdkw.hrmp.hrobs.common.hrobs.util.PersonAnalyzeUtil;
 ...
 * @return: null
 **/
public class PersonAnalyzeFormPlugin extends AbstractFormPlugin {
    ...
    public void afterCreateNewData(EventObject e) {
        ...
        localConfig.setTimeout(2 * 60 * 60);
        LocalMemoryCache localCache = CacheFactory.getCommonCacheFactory().$getOrCreateLocalMemoryCache("personAnalyze", "erManFileIds", localConfig);
        ...
        Date defaultEndDate = PersonAnalyzeUtil.getDefaultEndDate();
        //开始时间
        Date tdkwBegindate = dataEntity.getDate("tdkw_begindate");
        Date defaultBeginDate = PersonAnalyzeUtil.getDefaultBeginDate();

        if (!(tdkwEnddate.equals(defaultEndDate) && tdkwBegindate.equals(defaultBeginDate))) {
            dateFlag = true;
        }
        ...
        if (ObjectUtils.isNotEmpty(tdkw_employtype) && tdkw_employtype.size() != 0) {
            ...
            List<String> number = tdkw_employtype.stream().map(i -> ((DynamicObject) i.get(1)).getString("number")).collect(Collectors.toList());
            List<String> defaultValues = Arrays.asList("XY00001", "XY00005", "XY00007", "XY00008", "XY00009");
            Set<String> numberSet = new HashSet<>(number);
            Set<String> defaultValuesSet = new HashSet<>(defaultValues);
            boolean isEqual = numberSet.equals(defaultValuesSet);
            erManFilterFlag = !isEqual;
        }
        if (ObjectUtils.isNotEmpty(tdkw_job) && tdkw_job.size() != 0) {
            ...
            erManFileFiler.and("empposrel.tdkw_ranks.entryboid", QCP.in, entryboid);
            erManFilterFlag = true;
        }
        ...
        List<Long> erManFileIds;
        if (erManFilterFlag) {
            // 有条件直接查询
            erManFileIds = PeopleCountingUtils.getAllErManFileByPersonIds(erManFileFiler);
        } else {
            // 没条件从缓存中读取、即第一次进入
            erManFileIds = (List<Long>) localCache.get("PersonAnalyzeErManFileIds" + currentUserId);
            logger.info("从缓存读取人员分析erManFileIds成功");
            // 防止报错
            if (CollectionUtils.isEmpty(erManFileIds)) {
                logger.info("从缓存读取人员分析erManFileIds失败");
                erManFileIds = PeopleCountingUtils.getAllErManFileByPersonIds(erManFileFiler);
            }
        }

        logger.info("erManFileIds.size" + erManFileIds.size());

        // 人事业务档案
        boolean erManFileEduAndPoliIdsFlag = false;
        ...
        if (ObjectUtils.isNotEmpty(tdkw_education) && tdkw_education.size() != 0) {
            ...
            erManFileEduAndPoliIdsFlag = true;
        }
        ...
        qFilter.and("enddate", QCP.large_equals, dataEntity.getDate("tdkw_begindate"));
        if (!(tdkwBegindate.equals(defaultBeginDate))) {
            erManFileEduAndPoliIdsFlag = true;
        }
        //组织过滤
        //非全集团才需要对组织过滤
        ...
        List<DynamicObject> personEmpposorgrel;
        if (erManFileEduAndPoliIdsFlag) {
            // 有条件直接查询
            personEmpposorgrel = QueryServiceHelper.query("hrpi_empposorgrel", "person.id,startdate,sysenddate,adminorg.id", qFilter.toArray());
        } else {
            // 没条件从缓存中读取、即第一次进入
            personEmpposorgrel = ((List<DynamicObject>) localCache.get("PersonAnalyzePersonEmpposorgrel" + currentUserId));
            logger.info("从缓存读取人员分析personEmpposorgrel成功");
            // 防止报错
            if (CollectionUtils.isEmpty(personEmpposorgrel)) {
                logger.info("从缓存读取人员分析personEmpposorgrel失败");
                personEmpposorgrel = QueryServiceHelper.query("hrpi_empposorgrel", "person.id,startdate,sysenddate,adminorg.id", qFilter.toArray());
            }
        }
        logger.info("personEmpposorgrel.size" + personEmpposorgrel.size());
        ...
        if (dateFlag) {
            ...
            // 没条件从缓存中读取、即第一次进入
            personIds = ((List<Long>) localCache.get("PersonAnalyzePersonIds" + currentUserId));
            personDy = ((List<DynamicObject>) localCache.get("PersonAnalyzePersonDy" + currentUserId));
            logger.info("从缓存读取人员分析personIds成功");
            //  防止报错  personIds、personDy
            if (CollectionUtils.isEmpty(personIds) || CollectionUtils.isEmpty(personDy)) {
                logger.info("从缓存读取人员分析personIds失败");
                personIds = new ArrayList<>();
                personDy = new ArrayList<>();
                for (DynamicObject dynamicObject : personEmpposorgrel) {
                    Date startDate = dynamicObject.getDate("startdate");
                    Date endDate = dynamicObject.getDate("sysenddate");
                    Long personId = dynamicObject.getLong("person.id");
        ...
        logger.info("personIds.size" + personIds.size());
        logger.info("personDy.size" + personDy.size());
...
//            //查询出hr组织数据并进行排序
//            haos_adminorghr = QueryServiceHelper.query("haos_adminorghr", "id,name", filter.toArray(), "sortcode");
//            finalOrgIds = haos_adminorghr.stream().map(i -> i.getLong("id")).collect(Collectors.toList());
//        } else {
//            finalOrgIds = (List<Long>) localCache.get("finalOrgIds" + currentUserId);
//
//            haos_adminorghr = (DynamicObjectCollection) localCache.get("haos_adminorghr" + currentUserId);
...
//
//
//        if (finalOrgIds.isEmpty()) {
//            finalOrgIds = Collections.singletonList((Long) tdkwOrg.getPkValue());
//            logger.info("finalOrgIds为空缓存中读取finalOrgIds" + finalOrgIds.toString());
//        }
        ...
        filter.and("enable", QCP.equals, "1");
        //查询出hr组织数据并进行排序
        DynamicObjectCollection haos_adminorghr = QueryServiceHelper.query("haos_adminorghr", "id,name", filter.toArray(), "sortcode");
        List<Long> finalOrgIds = haos_adminorghr.stream().map(i -> i.getLong("id")).collect(Collectors.toList());

        if (personIds.size() == 0) {
            return;
        }
        DynamicObjectCollection personObjects;
        if (dateFlag) {
            DataSet personInfos = PersonInfoUtil.queryPersonInfo(personIds, this.getClass().getName());
            personObjects = ORM.create().toPlainDynamicObjectCollection(personInfos);
        } else {
            personObjects = ((DynamicObjectCollection) localCache.get("PersonAnalyzePersonObjects" + currentUserId));
            logger.info("从缓存读取人员分析personObjects成功");
            // 防止报错
            if (CollectionUtils.isEmpty(personObjects)) {
                logger.info("从缓存读取人员分析personObjects失败");
                DataSet personInfos = PersonInfoUtil.queryPersonInfo(personIds, this.getClass().getName());
                personObjects = ORM.create().toPlainDynamicObjectCollection(personInfos);
            }
        }
        logger.info("personObjects.size" + personObjects.size());


        List<DynamicObject> collects = new ArrayList<>();
        ...
        if (tdkwOrg.getLong("id") != 100000 || !hasAllOrgPerm) {
            collects = personDy.stream().filter(i -> i.getDate("startdate")
            ...
            for (Long finalOrgId : finalOrgIds) {
                ...
                List<Long> dirOrgPersons = collects.stream().filter(i -> allBelowHROrg.contains(i.getLong("adminorg.id")))
            ...
            collects = ((List<DynamicObject>) localCache.get("collects" + currentUserId));
            xNames = ((List<String>) localCache.get("xNames" + currentUserId));
            yDatas = ((List<Number>) localCache.get("yDatas" + currentUserId));
            orgForPersons = ((Map<Long, List<Long>>) localCache.get("orgForPersons" + currentUserId));
            orgNameMap = ((Map<Long, DynamicObject>) localCache.get("orgNameMap" + currentUserId));
            dirOrgPersonAll = ((List<Long>) localCache.get("dirOrgPersonAll" + currentUserId));
            if (CollectionUtils.isEmpty(collects)) {
                ...
                orgForPersons = new HashMap<>();
                orgNameMap = new HashMap<>();
                dirOrgPersonAll = new ArrayList<>();
                for (Long finalOrgId : finalOrgIds) {
                    //获取当前层级组织及其下级组织
                    List<Long> allBelowHROrg = HRRoleAndPersonUtils.getHROrgIds(finalOrgId.toString());
                    List<Long> dirOrgPersons = collects.stream().filter(i -> allBelowHROrg.contains(i.getLong("adminorg.id")))
                    ...
                    yDatas.add(dirOrgPersons.size());
                    this.getPageCache().put(this.getClass().getName() + orgObject.getString("name"), dirOrgPersons.toString());
                    logger.info(this.getClass().getName() + orgObject.getString("name") + "缓存数据" + dirOrgPersons.size());
                }
            }
        }
        ...
        if (dirOrgPersonAll.size() != 0) {
            ...
            logger.info(this.getClass().getName() + dataEntity.getDynamicObject("tdkw_org").getString("name") + "缓存数据" + dirOrgPersonAll.size());
        ...
        this.drawChartPoint(pointLineChart, model, personEmpposorgrel, tdkwBegindate, tdkwEnddate, erManFileIds);
        String tdkwPiecharttype = dataEntity.getString("tdkw_piecharttype");
        PieChart pieChart = this.getControl("tdkw_piechartap");
        this.drawChart(pieChart, tdkwPiecharttype, personObjects);
        this.getView().updateView("tdkw_piechartap");
        // 绘制总人数折线图
        drawHeadCount(histogramChart, xNames, yDatas);
        //开始绘制人员平均年龄柱状图
        ...
        this.drawChartResume(histogramChartResume, haos_adminorghr, finalOrgIds, orgNameMap);
        //开始绘制政治面貌柱状图
    ...
    public void click(EventObject evt) {
        ...
        if ("tdkw_pointlinechartap".equals(key)) {
            ...
            ReportShowParameter showParameter = new ReportShowParameter();
            showParameter.setFormId("tdkw_personinforpt");
            showParameter.setCaption("人员结构分布");
            showParameter.getOpenStyle().setShowType(ShowType.MainNewTabPage);
            showParameter.setHasRight(true);
        ...
        //总人数柱状图
        else if ("tdkw_totalnumchartap".equals(key)) {
            ChartClickEvent e = (ChartClickEvent) evt;
            String name = e.getName();
            if (ObjectUtils.isEmpty(name)) {
                return;
        ...
        //平均年龄柱状图
        else if ("tdkw_ageavgchartap".equals(key)) {
            ChartClickEvent e = (ChartClickEvent) evt;
            String name = e.getName();
            if (ObjectUtils.isEmpty(name)) {
                return;
            }
            if ("XXX集团".equals(name)) {
                return;
            }
            ReportShowParameter showParameter = new ReportShowParameter();
            showParameter.setFormId("tdkw_personinforpt");
            showParameter.setCaption("平均年龄");
            showParameter.getOpenStyle().setShowType(ShowType.MainNewTabPage);
            showParameter.setHasRight(true);
            showParameter.setCustomParam("personIds", this.getPageCache().get(this.getClass().getName() + name));
            this.getView().showForm(showParameter);
        ...
        //简历完整度柱状图
        else if ("tdkw_resumechartap".equals(key)) {
            ChartClickEvent e = (ChartClickEvent) evt;
            String name = e.getName();
            if (ObjectUtils.isEmpty(name)) {
                return;
            ...
            showParameter.setCaption("人员信息简历完整度");
            showParameter.getOpenStyle().setShowType(ShowType.MainNewTabPage);
            DynamicObject dataEntity = this.getModel().getDataEntity();
            Date tdkwEnddate = dataEntity.getDate("tdkw_enddate");
        ...
        else if ("tdkw_politicalchartap".equals(key)) {
            ChartClickEvent e = (ChartClickEvent) evt;
            String name = e.getName();
            if (ObjectUtils.isEmpty(name)) {
                return;
            ...
            showParameter.setCaption("中高层党员占比报表");
            showParameter.getOpenStyle().setShowType(ShowType.MainNewTabPage);
            showParameter.setHasRight(true);
            showParameter.setCustomParam("personIds", this.getPageCache().get(this.getClass().getName() + name));
            this.getView().showForm(showParameter);
        } else if ("tdkw_querybtn".equals(key)) {
            DynamicObject dataEntity = this.getModel().getDataEntity();
            //开始时间
            Date tdkwBegindate = dataEntity.getDate("tdkw_begindate");
            //结束时间
            ...
            this.getView().updateView("tdkw_piecharttype");
    ...
    public void drawChartPoint(PointLineChart pointLineChart, IDataModel model, List<DynamicObject> personEmpposorgrel, Date tdkwBegindate,
                               Date tdkwEnddate, List<Long> erManFileIds) {
        boolean isX = true;
        // 创建分类轴，X轴方式展现
        ...
        // 创建折线并赋值
        this.createLineSeries(pointLineChart, "人员", contructValueData(monthList, personEmpposorgrel, tdkwBegindate, tdkwEnddate, erManFileIds), "");
     ...
     */
    private List<BigDecimal> contructValueData(List<Date> monthList, List<DynamicObject> personEmpposorgrel, Date tdkwBegindate,
                                               Date tdkwEnddate, List<Long> erManFileIds) {
        ...
        for (Date date : monthList) {
            erManFileIds = personIds.get(date);
...
//        barseries.setColor("#d0332f");
    ...
    /**
     * 绘制简历完整度柱状图表（11.15需求变更，要求进行信息稽核）
     *
     * @param histogramChart
     * @param haos_adminorghr
     * @param finalOrgIds
     * @param orgNameMap
     */
    public void drawChartResume(HistogramChart histogramChart, DynamicObjectCollection haos_adminorghr, List<Long> finalOrgIds, Map<Long, DynamicObject> orgNameMap) {
        long start = System.currentTimeMillis();
...
//        boolean hasAllOrgPerm = result.isHasAllOrgPerm();
        ...
        if (tdkwOrgId != 100000 || !hasAllOrgPerm) {
            //非全集团需要获取组织信息
            allPersonByOrg = PersonInfoUtil.getAllPersonByOrg((Long) tdkwOrgId);
...
//        boolean hasAllOrgPerm = result.isHasAllOrgPerm();
    ...
    public BigDecimal getSuperiorOrg() {
        ...
        DataSet ermanFileData = ORM.create().queryDataSet(this.getClass().getName(), "hspm_ermanfile", "person,empposrel.company as company," +
                "person.id as personid,person.name as personname,empposrel.position," +
