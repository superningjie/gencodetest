 ...
 * @date 2023/10/25
 */
public class BusiRosterLeftTreePlugin extends AbstractReportPlugin {
    ...
    public void beforeQuery(ReportQueryParam queryParam) {
        ...
        if (queryParam.getCustomParam().get("orgIds") == null) {
            ...
            Long rootId = Long.valueOf(initAllOrgTree().getId());
            if (rootId == 100000L) {
                param.put("orgIds", Collections.singletonList(rootId));
                logger.info("点击左树传递的orgids为:" + rootId);
            } else {
                param.put("orgIds", result.getHasPermOrgs());
                logger.info("点击左树传递的orgids为:" + result.getHasPermOrgs());
            }
            queryParam.setCustomParam(param);
        } else {
            logger.info("点击左树传递的orgids为:" + queryParam.getCustomParam().get("orgIds"));
        }
    ...
    public void preOpenForm(PreOpenFormEventArgs e) {
        ...
        switch (formId) {
            ...
            case "tdkw_busiunitrosterrpt":
                ...
                break;
            case "tdkw_haos_divisionreport":
                orgString = "tdkw_division_pc";
                property = System.getProperty("hrobs.properties.structprojectnumber");
                qFilter = new QFilter("number", QCP.equals, property);
                dynamicObject = BusinessDataServiceHelper.loadSingle("haos_structproject", "id", new QFilter[]{qFilter});
                list = new ArrayList<>();
                list.add(dynamicObject.getPkValue().toString());
                perParams.put("struct_project_ids", list.toString());
                formShowParameter.setCustomParam("struct_project_ids", list.toString());
