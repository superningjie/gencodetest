...
public class RosRepository {
    ...
    public static QFilter getOrgFilter(String fieldName) {
        ...
        HasPermOrgResult result = PermissionServiceHelper.getAllPermOrgs(RequestContext.get().getCurrUserId(), "hspm", "tdkw_roster_report", "47150e89000000ac");
        if (result == null) {
    ...
    public static QFilter getAdminOrgFilter(String fieldName) {
        ...
        AuthorizedOrgResult result = (AuthorizedOrgResult) HRMServiceHelper.invokeHRMPService("hrcs", "IHRCSBizDataPermissionService", "getAuthorizedAdminOrgsF7", new Object[]{RequestContext.get().getCurrUserId(), "hrpi", "tdkw_roster_report", "47150e89000000ac", "adminorg"});
        if (result == null) {
            return QFilter.isNull("id");
        } else {
    ...
    private static void andCommonFilter(FilterInfo filterInfo, QFilter filter) {
        ...
        QFilter commonFilter = (QFilter) filterInfo.getCommFilter().get("hspm_hmcquery");
    ...
    private static void andDataRuleFilter(QFilter filter) {
        //TODO "hspm_qbggquery 改为要读取的查询配置编码
        QFilter dataRuleFilter = (QFilter) HRMServiceHelper.invokeHRMPService("hrcs", "IHRCSDataPermissionService", "getDataRule", new Object[]{RequestContext.get().getCurrUserId(), "1WXBPN7+OHJZ", "hspm_hmcquery", "47150e89000000ac", Collections.emptyMap()});
