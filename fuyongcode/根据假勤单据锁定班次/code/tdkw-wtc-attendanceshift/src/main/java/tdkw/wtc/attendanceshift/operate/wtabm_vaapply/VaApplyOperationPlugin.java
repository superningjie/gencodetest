...
import tdkw.wtc.attendanceshift.operate.wtte_form.util.StandardDateUtil;
...
/**
 *
 * @description 休假申请操作插件
 * @author ljl
 * @date 2024/4/12 14:54
 **/
public class VaApplyOperationPlugin extends AbstractOperationServicePlugIn implements Plugin {

    private static final Log logger = LogFactory.getLog(VaApplyOperationPlugin.class);
    ...
    // 休假方式-全天
    private static final String DATE_METHOD = "2";
    ...
    public void onPreparePropertys(PreparePropertysEventArgs e) {
        ...
        e.getFieldKeys().add("entrystartdate");
        e.getFieldKeys().add("entryenddate");
    }
    ...
    public void beginOperationTransaction(BeginOperationTransactionArgs e) {
        ...
        for (DynamicObject dynamicObject : dataEntities) {
            ...
            int entryvatime = dynamicObject.getDynamicObjectCollection("entryentity")
                    .stream().map(x -> x.getInt("entryvatimehour")).reduce(0,Integer::sum);
            queryData(dynamicObject,personrosterCollection,addPersonrosterCollection,operationKey,entryvatime,false);
            List<String> status = new ArrayList<>();
            Collections.addAll(status,WFAUDITNOTPASS,UNSUBMIT,WFREJECTTOSUBMIT,UNAUDIT);
            if (operationKey.equals(SUBMIT) || operationKey.equals(SUBMITEFFECT)){//提交、保存-锁定计划班次和实际班次
                isclock = 1;
            ...
            if ((operationKey.equals(AUDIT) || status.contains(operationKey)||operationKey.equals(SUBMITEFFECT)) && parentid != 0) {
                DynamicObject parentData = BusinessDataServiceHelper.loadSingle(parentid, "wtabm_vaapply");
                queryData(parentData,personrosterCollection,addPersonrosterCollection,operationKey,entryvatime,true);
            }
    ...
    private void queryData(DynamicObject dynamicObject,List<DynamicObject> personrosterCollection,
                           List<DynamicObject> addPersonrosterCollection,String operationKey,int entryvatime,Boolean flag){
        ...
        for (DynamicObject entity : entryentity) {
            ...
            Date entrystartdate = entity.getDate("entrystartdate");

            Date entryenddate = entity.getDate("entryenddate");
            //获取对应天的归属日期
/*            WtomHelper wtomHelper = new WtomHelper();
            ...
            String entryStartMethod = entity.getString("entrystartmethod");
            String entryEndMethod = entity.getString("entryendmethod");
            Date startStandardDate = StandardDateUtil.getStandardDate(entrystartdate, boid, false);
            Date endStandardDate = StandardDateUtil.getStandardDate(entryenddate, boid, true);
            LocalDate start = DATE_METHOD.equals(entryStartMethod) ? entrystartdate.toInstant().atZone(ZoneId.systemDefault()).toLocalDate():startStandardDate.toInstant().atZone(ZoneId.systemDefault()).toLocalDate();
            LocalDate end = DATE_METHOD.equals(entryEndMethod) ? entryenddate.toInstant().atZone(ZoneId.systemDefault()).toLocalDate():endStandardDate.toInstant().atZone(ZoneId.systemDefault()).toLocalDate();

            logger.info("休假单-班次锁定开始日期"+start);
            logger.info("休假单-班次锁定结束日期"+end);
            ...
            if (flag){
                if (operationKey.equals(AUDIT)||operationKey.equals(SUBMITEFFECT)){//审核通过-锁定当前变更单的排班，解锁原单的排班
                    ...
                    for (DynamicObject personroster:personrosterList) {
                        if (!changeIds.contains(personroster.get("id")) || entryvatime == 0){
