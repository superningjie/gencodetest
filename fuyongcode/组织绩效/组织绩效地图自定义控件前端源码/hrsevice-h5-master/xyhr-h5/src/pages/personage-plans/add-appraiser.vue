<template>
  <div class="add-appraiser">
    <div class="header">
      ...
      <div class="selected-box">
        <van-collapse v-model="activeCollapse">
          <!-- 基本信息 -->
          <van-collapse-item name="selected" class="infos-item">
            <template #title>
              <div class="title">已选人员</div>
            </template>
            <div class="tags">
              <van-tag
                v-for="(ite, index) in result"
                :key="index"
                closeable
                size="large"
                color="#EFF1F4"
    ...
    <div class="content">
      <van-checkbox-group v-model="result" v-if="appraiserList.length">
        <van-list
          v-model="loading"
          :finished="finished"
          finished-text="没有更多了"
  ...
  methods: {
    ...
    handleSubmit() {
      if (this.result.length) {
        ...
        addReplenishers(params).then((res) => {
          if (res.status == 200 && res.data.statusCode == "200") {
            Toast.success("设置成功");
            setTimeout(() => {
              em.closeWindow();
