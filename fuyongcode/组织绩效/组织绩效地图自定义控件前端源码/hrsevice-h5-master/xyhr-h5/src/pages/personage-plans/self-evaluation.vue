          ...
          <van-collapse-item name="Situation" class="infos-item">
            <template #title>
              <div class="title-box">
                ...
                ></van-image>
                <div class="title">员工总体自评</div>
              </div>
            </template>
            <div class="situation-content-info">
              <div>
                <p>
                  完成情况<span style="color: red">*</span>：<span
                    v-if="selfEvaluate.finishStatus"
                    >{{ selfEvaluate.finishStatus }}</span
                  >
                </p>
                <van-radio-group
                  v-if="!selfEvaluate.finishStatus"
                ...
                <p style="margin: 10px 0">完成情况评价：</p>
                <van-field
                  autosize
                  rows="1"
                  type="textarea"
                  ...
                  :disabled="!!selfEvaluate.finishStatus"
                />
              </div>
            </div>
          </van-collapse-item>
...
export default {
  ...
  methods: {
    ...
    // 提交
    handleSubmit() {
      if (this.detailInfo.isLook) {
        this.submitEvaluateFinish();
        return;
      }
      this.$refs.form
        .validate()
    ...
    Evaluation() {
      let params = {
        param: {
          idpId: this.planId,
          number: this.personNum,
          evaluate: this.mentorEvaltList[0].evaluate,
        },
      };
    ...
    submitEvaluateFinish() {
      ...
      let params = {
        param: {
          ...
          finishEvaluation: this.resultValue,
        },
        type: 3,
      };
      submitFormEvaluate(params).then((res) => {
        if (res.status == 200 && res.data.statusCode == "200") {
          this.$toast.success("提交成功");
          setTimeout(() => {
            em.closeWindow();
