<template>
  <div class="confirm">
    <!-- <div class="header">
      <van-nav-bar>
        <template #title>
          <span class="navbar-title">期间任务自评</span>
        </template>
        <template #left>
              ...
              <van-form ref="form">
                ...
                >
                  ...
                  <div class="list-content">
                    <div class="content-item" v-if="ite.taskStatus == '已评价'">
                      <span class="item-label"> 评价结果 </span>
                      <span class="item-value"> {{ ite.eltResult }} </span>
                    </div>
                    <div class="content-item" v-if="ite.taskStatus == '已评价'">
                      <span class="item-label"> 评价结果描述 </span>
                      <span class="item-value"> {{ ite.eltResultDes }} </span>
                    </div>
                    <div class="content-item" v-if="ite.taskStatus == '已评价'">
                      <span class="item-label"> 评价人 </span>
                      <span class="item-value"> {{ ite.appraiser }} </span>
                    </div>
                    <div class="content-item" v-if="ite.taskStatus == '已评价'">
                      <span class="item-label"> 评价时间 </span>
                      <span class="item-value"> {{ ite.time }} </span>
                    </div>
                    ...
                    <div class="content-item">
                      <van-field name="finish" :rules="finish">
                        <template #input>
                          <van-radio-group
                            v-model="evaluateList[index].selfResult"
                    ...
                    <div class="content-item">
                      <van-field
                        v-model="evaluateList[index].selfResultDes"
                        :disabled="
                          ite.taskStatus == '待评价' ||
                          ite.taskStatus == '已评价'
                        "
...
<script>
import { selfEvaluation, EvaluateByList, getEvaluate } from "@/libs/api.js";
export default {
  data() {
    return {
      ...
      evaluateList: [],
      finish: [
        { required: true, message: "请选择自评结果", trigger: "onBlur" },
      ],
      showAll: false,
      idpId: null,
      taskId: null,
  ...
  methods: {
    handleSubmit() {
      this.$refs.form
        .validate()
        .then(() => {
          this.selfList = this.evaluateList.map((ite) => {
            return {
              taskId: ite.id,
              selfResult: ite.selfResult,
              selfResultDes: ite.selfResultDes,
            };
          });
          let params = {
            param: {
              idpId: this.idpId,
              number: this.number,
              selfData: this.selfList,
            },
          };
          selfEvaluation(params).then((res) => {
