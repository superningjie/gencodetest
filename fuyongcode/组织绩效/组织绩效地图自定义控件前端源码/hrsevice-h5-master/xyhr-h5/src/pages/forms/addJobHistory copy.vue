<template>
  <div class="myProfile editJobHistory">
    <van-nav-bar :title="'新增'+title" left-arrow class="navStyle" @click-left="goback" />
    <van-cell-group class="group">
      <template  v-for="item in info.fieldTemplates">
        <van-cell :title="item.name" v-if="item.showFlag=='Y'" is-link
          :required="item.mustFlag=='Y'" :key="item.code">
          <template #right-icon>
            <template
              v-if="item.dataType==3 || item.dataType==5 || item.dataType==6  || item.dataType==20 || item.dataType==4 || item.dataType==20 || item.dataType==4 || item.dataType==101  || item.dataType==102 || item.dataType==100">
              <span @click="popupClick(item)" v-if="item.canEditFlag =='Y'" style="display:inline-block;max-width:50%;">
                {{item.showValue?item.showValue==""?"请选择":item.showValue:"请选择"}}
              </span>
              <span v-else-if="item.canEditFlag =='N'" :class="item.canEditFlag =='Y'?'':'mygray'">
                {{item.showValue==""?"请选择":item.showValue}}
              </span>
              <van-icon name="arrow" style="line-height:24px;" :class="item.canEditFlag =='Y'?'':'mygray'" />
            </template>
            <template v-else>
              <van-field v-model="item.value" :type="item.dataType==1?'digit':item.dataType==2?'number':'text'"
                :readonly="item.canEditFlag !='Y'" placeholder="请填写" @input="oninput(item)" />
            </template>
          </template>
        </van-cell>
      </template>
      <van-popup v-model="showUserlist" :style="{ width: '100%', height: '100%' }" position="right">
        <myuserlist @closeList="showUserlist=false" @pick="pick" :initArr='person' :plist='plist' v-if="showUserlist" />
      </van-popup>
    </van-cell-group>
    ...
    <van-popup v-model="showPicker" class="pickerPopup" :overlay-style="{'background': 'rgba(120,120,120,.5)'}">
      <van-search shape="round" v-if="pickList.length>20 || keyword.length>0" style="padding:0;" v-model="keyword"
        :clearable="false" placeholder="输入关键字搜索" @search="onsearch" />
      <template v-for="(item,index) in datas.pickData">
        <label @click="pick(item)" :key="index">
    ...
    <van-popup v-model="showCalender" class="calenderPop" :overlay-style="{'background': 'rgba(120,120,120,.5)'}">
      <Calendar :default-date="new Date(thisdate)" :is-show-week-view="false" :mark-date="markDate" v-if="showCalender"
        @confirm="dateConfirm"></Calendar>
    </van-popup>
    <profiletree v-if="showFilterPicker" :showFilterPicker="showFilterPicker" @isshowFilterPicker="isshowFilterPicker"
      @clickSearch="clickSearch" :columns="profileTree" />
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
      dateConfirm(date) {
        if (this.isdatehide) {
          this.info.fieldTemplates.map(item => {
            if (item.code == this.code) {
              item.showValue = date.year + '-' + (date.month < 9 ? '0' + (date.month + 1) : date.month + 1) + '-' +
                (date.day < 10 ? '0' + date.day : date.day)
              item.value = date.year + '-' + (date.month < 9 ? '0' + (date.month + 1) : date.month + 1) + '-' + (
                date.day < 10 ? '0' + date.day : date.day)
              item.alterFlag = 'Y'
        ...
        if (!this.editDataList[value.code]) {
          ...
          if (value.canEditFlag == "Y" && value.showFlag == "Y") {
            ...
            if (value.dataType == 5) {
              ...
              this.getReference(datas, value.code)
            } else if (value.dataType == 20 || value.dataType == 3 || value.dataType == 101 || value.dataType == 102 ||
              value.dataType == 100) {
              this.toast.clear()
              ...
              this.editDataList[this.code] = {
                references: this.initList,
                dataType: 'LIST'
              }
      ...
      confirm() {
        Dialog.confirm({
          ...
          .then(() => {
            ...
            this.parentInfo.records.push({
              fields: this.info.fieldTemplates,
              editType: 2
            })
            this.isedit()
          })
