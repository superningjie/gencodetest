    ...
    >
      ...
      >
        <van-tab title="行政架构" :name="'xz'">
          <trainJurisdictionTreeChild
            ref="trainJurisdictionTreeChild"
            :name="'xz'"
...
<script>
import { orgsTrainAuthList } from "@/libs/api.js";
import { Toast } from "vant";
import trainJurisdictionTreeChild from "@/components/trainJurisdictionTreeChild";
export default {
  name: "trainJurisdictionTree",
  data() {
    return {
      activeTab: "xz",
      orgsCommonList: [],
  ...
  components: { trainJurisdictionTreeChild },
  watch: {
    showFilterPicker: {
      deep: true,
  ...
  methods: {
    orgsCommon() {
      ...
      orgsTrainAuthList(this.data).then((res) => {
        if (res.data.data) {
          if (!this.initBack) {
            Toast.clear();
          }
          ...
          if (res.data.data.length == 1 && res.data.data[0].children) {
            this.$nextTick(() => {
              this.$refs.trainJurisdictionTreeChild.gonextPage(
                res.data.data[0]
              );
              this.$nextTick(() => {
                if (this.initBack) {
