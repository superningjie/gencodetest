      ...
      <van-cell is-link v-for="(item, index) in showData" :key="index">
        <template #title>
          <div class="titleFramework" @click="clickSearch(item)">
            ...
            <span v-if="item.psncount && item.psncount != -1">({{ item.psncount }}人)</span>
          </div>
        </template>
        <template #right-icon>
...
<script>
import { orgsManagerTree, orgsBusinessTree ,teamOrgsManagerTree} from "@/libs/api.js"
...
export default {
  ...
  methods: {
    init(jobInfo) {
      this.workPosition = this.workPosition || localStorage.getItem('workPosition');
