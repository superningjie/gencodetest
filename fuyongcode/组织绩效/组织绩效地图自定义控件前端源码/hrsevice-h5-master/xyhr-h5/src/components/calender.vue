<template>
	<div :class="'calender ' + name">
		...
		<div :class="'monthPage ' + (pickRange?'range':'single')">
			<ul class="title">
		...
		methods: {
			pick(day) {
				...
				if(this.pickRange) {
					...
					this.pickDate.push(`${this.year}-${this.month}-${day}`);
					if(this.pickDate.length==2) {
						// 对选择的日期范围进行排序
						let day1 = new Date(this.pickDate[0]);
						let day2 = new Date(this.pickDate[1]);
						if(day1>day2) { this.pickDate.reverse(); }
						this.$emit('onpick', this.pickDate);
					}
				}else{
					this.pickDate[0] = `${this.year}-${this.month}-${day}`;
					this.day = day;
			...
			onMounted() {
				...
				if(this.date) {
					if(this.pickRange) { this.pickDate = this.date; }
					var dateArray;
					if(this.date[0].indexOf('/')>0) {
						dateArray=this.date[0].split('/');
					}else if(this.date[0].indexOf('-')>0) {
						dateArray=this.date[0].split('-');
					}
			...
			displayDate() {
				if(this.pickRange) {
					if(!this.pickDate[0]) {return '请选择';}
					let day1 = this.pickDate[0].split('-');
					let day2 = this.pickDate[1]?this.pickDate[1].split('-'):false;
					if(day2) {
						if(day1[0]==day2[0]) {
							if(day1[1]==day2[1]) {
								if(day1[2]==day2[2]) {
									return `${day1[0]}年${day1[1]}月${day1[2]}日当天`;
								}else{
									return `${day1[0]}年${day1[1]}月${day1[2]}日 - ${day2[2]}日`;
								}
							}else{
								return `${day1[0]}年${day1[1]}月${day1[2]}日 - ${day2[1]}月${day2[2]}日`;
							}
						}else{
							return `${day1[0]}年${day1[1]}月${day1[2]}日 - ${day2[0]}年${day2[1]}月${day2[2]}日`;
						}
					}else{
						return `${day1[0]}年${day1[1]}月${day1[2]}日 - ...`;
					}
				}else{
					if(this.date[0]) {
						let day = this.date[0].split('-');
