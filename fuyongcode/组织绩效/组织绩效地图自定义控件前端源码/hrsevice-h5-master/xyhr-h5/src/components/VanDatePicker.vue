   ...
   onChange(picker, value, index) {
    ...
    if (finallyVal.slice(0,len).join("")>finallyVal.slice(len+1).join("")){
     ...
     if (new Date(finallyVal.slice(0,len).join("-")+' 00:00:00').getTime()===this.maxDate.getTime()) {
      picker.setColumnValues(4,_this.monthArr.slice(0,maxMonth+1));
      // picker.setColumnValues(6,_this.dayArr.slice(0,maxDay));
     }
    }
    // else{
    //     let a = new Date(finallyVal.slice(0,len).join("-")+' 00:00:00').getTime();
    //     let b = new Date(finallyVal.slice(len+1).join("-")+' 00:00:00').getTime();
    //     if(parseInt(Math.abs(a-b)/1000/60/60/24) > 366){
    //       var halfYear = 365 * 24 * 3600 * 1000;
    //       var pastResult = b - halfYear;
    //       let c = new Date(pastResult);
    //       picker.setColumnValue(0,c.getFullYear()+'');
    //       picker.setColumnValue(1,('0'+(c.getMonth() + 1)).slice(-2));
    //     }
    // }
