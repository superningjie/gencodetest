  ...
  <van-popup v-model="isShow" position="right" class="popup-box">
    <div class="myProfile editJobHistory">
            ...
            <template #right-icon>
              ...
              >
                <span
                  ...
                  v-if="item.canEditFlag == 'Y'"
                  style="display:inline-block;max-width:50%;"
                >
                  {{ !item.showValue ? "请选择" : item.showValue }}
                </span>
                <span
                  v-else-if="item.canEditFlag == 'N'"
                  :class="item.canEditFlag == 'Y' ? '' : 'mygray'"
                >
                  {{ !item.showValue ? "请选择" : item.showValue }}
                </span>
                <van-icon
      ...
      <div class="bottombtn sticky">
        <div class="flex">
          <button
            v-if="parentInfo.canDeleteFlag && parentInfo.canDeleteFlag == 'Y'"
            @click="deleteData"
          >
            删除
          </button>
  ...
  methods: {
    ...
    showlistortree(code) {
      ...
      if (this.editDataList[code].dataType == "LIST") {
        ...
        // this.popupshow=true
        this.datas.pickData = JSON.parse(
          JSON.stringify(this.editDataList[code].references)
        );
        this.pickList = JSON.parse(
          JSON.stringify(this.editDataList[code].references)
        );
    ...
    confirm() {
      ...
      const data = {
        editType: this.info.recordId ? 1 : 2,
        approve: this.info.recordId ? true : false,
        recordId: this.info.recordId,
        fields: JSON.parse(JSON.stringify(this.info.fields))
      };
      console.log('editJob==',data)
      this.parentInfo.records.splice(this.insertIndex, 1, data);
      this.goback();
      // this.isedit()
    },
    deleteData() {
      Dialog.confirm({
        ...
        if(this.info.recordId){
          this.parentInfo.records[this.insertIndex].editType = 3
        }else{
          this.parentInfo.records.splice(this.insertIndex, 1);
        }
        this.goback();
      });
    },
