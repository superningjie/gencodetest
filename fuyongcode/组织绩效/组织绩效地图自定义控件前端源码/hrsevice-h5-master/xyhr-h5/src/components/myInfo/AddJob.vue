<template>
	<van-popup v-model="isShow" position="right" class="popup-box">
		<div class="myProfile editJobHistory">
			<van-nav-bar :title="'新增' + info.name" left-arrow class="navStyle" @click-left="goback" />
			<van-cell-group class="group">
				<template v-for="item in info.fieldTemplates">
					<van-cell :title="item.name" v-if="item.showFlag == 'Y'" is-link :required="item.mustFlag == 'Y'" :key="item.code">
						<template #right-icon>
							<template
								v-if="
									item.dataType == 3 ||
							...
							<template v-else>
								<van-field
									v-model="item.value"
									:type="item.dataType == 1 ? 'digit' : item.dataType == 2 ? 'number' : 'text'"
									:readonly="item.canEditFlag != 'Y'"
									placeholder="请填写"
									@input="oninput(item)"
								/>
							</template>
...
import { clone, getReference, personalInfoEdit, dateFormat } from '@/libs/api.js'
...
export default {
	...
	data() {
		return {
			...
			toast: '',
			ispipstate: false,
			originalData: '', // 源数据，每次新增打开都以此进行一次复制
		}
	},
	watch: {
		showUserlist: {
	...
	methods: {
		show() {
			this.info = JSON.parse(JSON.stringify(this.originalData)) // 每次都取新的
			this.isShow = true
		},
		init(data) {
		...
		confirm() {
			...
			const addDataString = JSON.stringify(this.info.fieldTemplates)
			const addData = JSON.parse(addDataString)
