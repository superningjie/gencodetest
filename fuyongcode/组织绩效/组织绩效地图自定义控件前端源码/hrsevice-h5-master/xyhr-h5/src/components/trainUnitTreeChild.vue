...
<template>
  <div class="administrativeFramework">
    <div class="group">
      <h2 class="popUpSearch" style="line-height: 24px">
        <span v-for="(item, index) in dataDetail.titleList" :key="index">
          ...
          <span style="font-size: 14px" v-else @click="goreload(item, index)">{{ item.name }}<van-icon name="arrow" style="line-height: 24px; margin: 0 3px; vertical-align: middle" /></span>
        </span>
      </h2>
      <van-cell is-link v-for="(item, index) in showData" :key="index">
        <template #title>
          <div class="titleFramework" @click="clickSearch(item)"><van-image class="iconFramework" :src="item.type == 'org' ? xingzhengjiagou : bumen" />{{ item.name }}</div>
        </template>
        <template #right-icon>
