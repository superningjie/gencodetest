...
<template>
    ...
    <div class="bar" ref="bar" style="height:50px;" v-if="!isClockin">
    <span class="checkedDate">{{checkedDateStr}}</span>
    </div>
        <div class="switcher" >
			<div class="prev" @click="prevYear" style="margin-right:10px;">
				<van-image style="width:16px;height:16px;" :src="prev"></van-image>
			</div>
			<div class="prev" @click="prevMonth">
				<van-icon name="arrow-left" />
			</div>
			<div class="center">{{checkedDateTitle}}</div>
			<div class="next" @click="yqnextMonth">
				<van-icon name="arrow" />
			</div>
			<div class="next" @click="nextYear" style="margin-left:10px;">
        ...
        <div :style="{'height': `${calendarGroupHeight}px`}" @touchend="touchEnd" @touchmove.stop.prevent="touchMove"
             ...
             class="calendar_group" ref="calendar">
            <ul :style="{transform: `translate3d(${-translateIndex*100}%, 0, 0)`}">
                <li :key="i"
                    ...
                    <div :style="{transform: `translate3d(0, ${calendarY}px, 0)`,transitionDuration: `${isTouching||isWeekSwitch?0:transitionDuration}s`}"
                         class="calendar_group_li__content">
                        <p :style="{height: `${calendarGroupHeight}px`, opacity: `${isShowWeek?0:1}`,transitionDuration: `${isTouching?0:transitionDuration}s`}"
                           class="calendar_month">
                            {{(i===1?monthOfCurrentShow:i===0?lastMonth:nextMonth)+1}}
                        </p>
                        <div :key="i + j" @click="clickCalendarDay(date)" class="calendar_item" ref="calendarItem"
                             v-for="(date, j) in item">
                            <p :class="{'calendar_day_checked': isCheckedDay(date), 'calendar_day_not':  isNotCurrentMonthDay(date,i)}"
                               class="calendar_day"
                               ref="calendarDay">
                                ...
                                <span v-if="markDate.length > 0" :style="{'background': markDateColor(date)}" class="calendar_dot"></span>
                            </p>
                        </div>
                    </div>
                </li>
...
<script>
    import calendar from "../utils/chinese-calendar"
    ...
    export default {
        ...
        mounted() {
            ...
            this.monthOfCurrentShow=this.defaultDate.getMonth()
        },
        watch: {
            weekStartIndex() {
            ...
            defaultDate: {
                handler(val) {
                    ...
                    this.$set(this.showcheckedDate, 'day', val.getDate());
        ...
        computed: {
            checkedDateTitle: function () {
                if (this.yearOfToday === this.showcheckedDate.year) {
                    return this.formatDate(new Date(this.showcheckedDate.year, this.showcheckedDate.month, this.showcheckedDate.day), 'yyyy年mm月');
                } else {
                    return this.formatDate(new Date(this.showcheckedDate.year, this.showcheckedDate.month, this.showcheckedDate.day), 'yyyy年mm月');
                }
            },
            checkedDateStr: function () {
                if (this.yearOfToday === this.checkedDate.year) {
                    return this.formatDate(new Date(this.checkedDate.year, this.checkedDate.month, this.checkedDate.day), 'yyyy年mm月dd日');
                } else {
                    return this.formatDate(new Date(this.checkedDate.year, this.checkedDate.month, this.checkedDate.day), 'yyyy年mm月dd日');
                }
            },
        ...
        methods: {
            prevYear(){
                this.lastMonthYear = this.lastMonthYear-1;
			...
			nextYear(){
                this.nextMonthYear = this.nextMonthYear+1;
            ...
            initDom() {//初始化日历dom
                this.$nextTick(() => {
                    ...
                    // this.calendarItemHeight=this.calendarItemHeight+10
                    this.calendarItemHeight=35
            ...
            today() {//今天
                ...
                this.$set(this.showcheckedDate, 'year', new Date().getDate());
            ...
            calculateCalendarOfThreeMonth(year = new Date().getFullYear(), month = new Date().getMonth()) {
                ...
                if (day > 30 || (day > 28 && month === 1)) {
                    ...
                    this.$set(this.showcheckedDate, 'day', this.daysOfMonth(year)[month]);
                ...
                this.$set(this.showcheckedDate, 'year', year);
                this.$set(this.showcheckedDate, 'month', month);
                ...
                if(this.controlTrigger && this.isClockin){
                    this.controlTrigger=false
                    this.$set(this.checkedDate, 'year', year);
                    this.$set(this.checkedDate, 'month', month);
                    console.log()
                    if(new Date().getFullYear()==year && new Date().getMonth()==month){
                        ...
                        this.$set(this.checkedDate, 'day', 1)
                    }

                    this.$emit("dateChange",{year:year,month:this.fillNumber(month+1)})
                }
            },
            ...
            clickCalendarDay(date) {//点击日历上的日期
                ...
                this.$set(this.checkedDate, 'day', date.day);
                    ...
                    this.$set(this.showcheckedDate, 'year', date.year);
                    this.$set(this.showcheckedDate, 'month', date.month);
                    this.$set(this.showcheckedDate, 'day', date.day);
                ...
                if (this.isShowWeek) {
                    ...
                    this.$emit('confirm', this.checkedDate);
            },
            ...
            getLastWeek() {//显示上一周
                ...
                this.showcheckedDate = this.lastWeek[this.selectedDayIndex];
                this.showWeek();
            },
            getNextWeek() {//显示下一周
                this.showcheckedDate = this.nextWeek[this.selectedDayIndex];
            ...
            markDateColor(date) {//当前日期是否需要标记
                ...
                if (color) {
                    color = this.isCheckedDay(date) ? 'white' :
                        (this.isToday(date) ? '#94CCFB' : '#d0332f');
                    return color;
                } else {
                    return undefined;
                }
            ...
            lunar(date) {
                let calendarTextObj = calendar.solar2lunar(date.year, date.month + 1, date.day);
                // 有节日
                // if (calendarTextObj.festival.length > 0) {
                //     return calendarTextObj.festival[0]
                // }
                // // 节气
                // if (calendarTextObj.isTerm) {
                //     return calendarTextObj.Term;
                // }
                // if (calendarTextObj.IDayCn === '初一') {
                //     return calendarTextObj.IMonthCn
                // }
                // // 无节日
