...
import kd.hr.haos.business.domain.projectteam.models.CalibratorBean;
import kd.hr.haos.business.domain.staff.valid.core.OrgStaffValidContext;
 ...
 * @Description : 编制申请保存插件
 * @Author : XYP
 * @Date: 2024-07-26 10:14
 */
public class BZchangeSaveOp extends AbstractOperationServicePlugIn {
    private static final Log LOGGER = LogFactory.getLog(BZchangeSaveOp.class);
    ...
    public void beforeExecuteOperationTransaction(BeforeOperationArgs e) {
        ...
        for (DynamicObject dynamicObject : dataEntities1) {
            this.savestaff(dynamicObject,e);
        }
    }

    void savestaff(DynamicObject dynamicObject,BeforeOperationArgs bef) {
        Long tdkw_staff = dynamicObject.getDynamicObject("tdkw_staff").getLong("id");
        DynamicObjectCollection tdkw_staffdimension = dynamicObject.getDynamicObjectCollection("tdkw_staffdimension");
        ...
        try (AutoCloseViewHandler viewHandler = AutoCloseViewHandler.of("haos_staff")) {
            ...
            DynamicObject compileinfo = childView.getModel().getDataEntity();
            OrgStaffValidContext orgStaffValidContext = new OrgStaffValidContext(compileinfo, "save");
            List<CalibratorBean> result = orgStaffValidContext.valid();
            String log = "";
            if (!CollectionUtils.isEmpty(result)) {
                for (CalibratorBean calibratorBean : result) {
                    String message = calibratorBean.getMessage();
                    log = message + ",\n" + log;
                }
            }
            if (!log.equals("")) {
                bef.setCancelMessage(log);
                bef.setCancel(true);
            }
        } catch (Exception e) {
            // 异常处理
